...
#define DEBUG(...)		do {debug_header("HOPPER 1 MSP: ", __VA_ARGS__);} while (0)
...
uint64_t hopper1msp_stk[2048 / 8];
const osThreadAttr_t Hopper1MSPTask_attributes = {
  .name = "Hopper1MSPTask",
  .stack_mem = &hopper1msp_stk[0],
  .stack_size = sizeof(hopper1msp_stk),
  .priority = (osPriority_t)osPriorityAboveNormal,
};
osThreadId_t Hopper1MSP_TaskHandle;

/* Private variables ---------------------------------------------------------*/
HopperMSPClass hopper_1_msp;
...
void HopperMSPClass::GPIO_Init(void)
{
	...
	GPIO_InitStruct.Pin = HOPPER_1_INHIBIT_Pin;
	HAL_GPIO_Init(HOPPER_1_INHIBIT_GPIO_Port, &GPIO_InitStruct);

	GPIO_InitStruct.Pin = HOPPER_1_METER_Pin;
	HAL_GPIO_Init(HOPPER_1_METER_GPIO_Port, &GPIO_InitStruct);

	GPIO_InitStruct.Pin = HOPPER_1_CREDIT_Pin;
	HAL_GPIO_Init(HOPPER_1_CREDIT_GPIO_Port, &GPIO_InitStruct);

	GPIO_InitStruct.Pin = HOPPER_1_COIN_EMPTY_Pin;
	HAL_GPIO_Init(HOPPER_1_COIN_EMPTY_GPIO_Port, &GPIO_InitStruct);

	PAYOUT_Pin_Set();
	GPIO_InitStruct.Pin = HOPPER_1_PAYOUT_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_OD;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_MEDIUM;
	HAL_GPIO_Init(HOPPER_1_PAYOUT_GPIO_Port, &GPIO_InitStruct);

  inhibit_pin_state = HAL_GPIO_ReadPin(HOPPER_1_INHIBIT_GPIO_Port, HOPPER_1_INHIBIT_Pin);
  meter_pin_state = HAL_GPIO_ReadPin(HOPPER_1_METER_GPIO_Port, HOPPER_1_METER_Pin);
  credit_pin_state = HAL_GPIO_ReadPin(HOPPER_1_CREDIT_GPIO_Port, HOPPER_1_CREDIT_Pin);
  coin_empty_pin_state = HAL_GPIO_ReadPin(HOPPER_1_COIN_EMPTY_GPIO_Port, HOPPER_1_COIN_EMPTY_Pin);
}
...
void HopperMSPClass::PAYOUT_Pin_Set(void) {
	HAL_GPIO_WritePin(HOPPER_1_PAYOUT_GPIO_Port, HOPPER_1_PAYOUT_Pin, GPIO_PIN_SET);
}

void HopperMSPClass::PAYOUT_Pin_Reset(void) {
  HAL_GPIO_WritePin(HOPPER_1_PAYOUT_GPIO_Port, HOPPER_1_PAYOUT_Pin, GPIO_PIN_RESET);
}

void HopperMSPClass::Hopper_1_MSP_Thread(void* argument)
{
	uint32_t flags;
  hopper_1_msp.Start = true;
	for (;;)
	{
    hopper_1_msp.Check_Alarm();
    
    MessageClass msg;
		osStatus_t status;
		status = hopper_1_msp.Messages.popMessageIn(&msg, 1);   // wait for message
		
    if (status == osOK) {
			//Coints out
			int32_t coint_out = msg.coins;
			hopper_1_msp.Out_Coins(&coint_out);
			msg.coins = coint_out;
      hopper_1_msp.Messages.pushMessageOut(&msg);
			osThreadYield();
		}
		flags = osEventFlagsWait(hopper_1_msp.Event_ID, HOPPER_FLAGS_MSK, osFlagsWaitAny, 0U);
		if (Check_CMSIS_Flags_Functions_Error_Codes(flags) == 0) {
//      if(flags&EventHopperMSP::INHIBIT_PIN_RISING){
//        DEBUG("NOISE ON LINE HOPPER_1_INHIBIT_Pin, RISING\n");
//      }
//      if(flags&EventHopperMSP::METER_PIN_RISING){
//        DEBUG("NOISE ON LINE HOPPER_1_METER_Pin, RISING\n");
//      }
//      if(flags&EventHopperMSP::CREDIT_PIN_RISING){
//        DEBUG("NOISE ON LINE HOPPER_1_CREDIT_Pin, RISING\n");
//      }
//			if (flags & EventHopperMSP::COIN_EMPTY_PIN_RISING) {
//				DEBUG("NOISE ON LINE HOPPER_1_COIN_EMPTY_Pin, RISING\n");
//			}
//      if(flags&EventHopperMSP::INHIBIT_PIN_FALLING){
//        DEBUG("NOISE ON LINE HOPPER_1_INHIBIT_Pin, FALLING\n");
//      }
//      if(flags&EventHopperMSP::METER_PIN_FALLING){
//        DEBUG("NOISE ON LINE HOPPER_1_METER_Pin, FALLING\n");
//      }
//      if(flags&EventHopperMSP::CREDIT_PIN_FALLING){
//        DEBUG("NOISE ON LINE HOPPER_1_CREDIT_Pin, FALLING\n");
//      }
//			if (flags & EventHopperMSP::COIN_EMPTY_PIN_FALLING) {
//				DEBUG("NOISE ON LINE HOPPER_1_COIN_EMPTY_Pin, FALLING\n");
//			}
//      if(flags&EventHopperMSP::INHIBIT_PIN_NOISE){
//        DEBUG("NOISE ON LINE HOPPER_1_INHIBIT_Pin, NOISE\n");
//      }
//      if(flags&EventHopperMSP::METER_PIN_NOISE){
//        DEBUG("NOISE ON LINE HOPPER_1_METER_Pin, NOISE\n");
//      }
//      if(flags&EventHopperMSP::CREDIT_PIN_NOISE){
//        DEBUG("NOISE ON LINE HOPPER_1_CREDIT_Pin, NOISE\n");
//      }
//			if (flags & EventHopperMSP::COIN_EMPTY_PIN_NOISE) {
//				DEBUG("NOISE ON LINE HOPPER_1_COIN_EMPTY_Pin, NOISE\n");
//			}
...
void HopperMSPClass::Check_Alarm(void) {
  ...
  alarm = false;
  if  (hopper_1_msp.coin_empty_pin_state != COIN_EMPTY_NORMAL_STATE)
  {
    osDelay(1);
    if  (hopper_1_msp.coin_empty_pin_state != COIN_EMPTY_NORMAL_STATE)
    {
  ...
  alarm = false;
  if  (hopper_1_msp.credit_pin_state != CREDIT_NORMAL_STATE)
  {
    osDelay(1);
    if  (hopper_1_msp.credit_pin_state != CREDIT_NORMAL_STATE)
    {
...
static void Debug_Pins(void) {
  DEBUG(
    "READ PIN:\n"
    "HOPPER_1_INHIBIT_Pin is %d - %s\n"
    "HOPPER_1_METER_Pin is %d - %s\n"
    "HOPPER_1_CREDIT_Pin is %d - %s\n"
    "HOPPER_1_COIN_EMPTY_Pin is %d - %s\n",
    hopper_1_msp.inhibit_pin_state,
    hopper_1_msp.inhibit_pin_state == INHIBIT_NORMAL_STATE ? "OK": "FAULT",
    hopper_1_msp.meter_pin_state,
    hopper_1_msp.meter_pin_state ? "OK": "FAULT",
    hopper_1_msp.credit_pin_state,
    hopper_1_msp.credit_pin_state ? "FAULT": "OK",
    hopper_1_msp.coin_empty_pin_state,
    hopper_1_msp.coin_empty_pin_state == COIN_EMPTY_NORMAL_STATE ? "OK": "FAULT"
  ); 
}
  ...
  uint32_t inhibit_signal_delay = 0;
	...
	int split_all_coints = *num;
		...
		if(split_all_coints >= 2){
			...
			msg.command = SPLIT_OUT_HOPPER_1;
			msg.pulses = 1;
...
void HopperMSPClass::EXTI_Callback()
{
  ...
  uint64_t flag;
    ...
    state = HAL_GPIO_ReadPin(HOPPER_1_INHIBIT_GPIO_Port, HOPPER_1_INHIBIT_Pin);
    if (state != hopper_1_msp.inhibit_pin_state)
    {
     if (state == GPIO_PIN_RESET) {
      ...
      hopper_1_msp.inhibit_pin_state = state;
    }


		state = HAL_GPIO_ReadPin(HOPPER_1_METER_GPIO_Port, HOPPER_1_METER_Pin);
		if (state != hopper_1_msp.meter_pin_state)
		{
			if (state == GPIO_PIN_RESET) {
			...
			hopper_1_msp.meter_pin_state = state;
		}


		state = HAL_GPIO_ReadPin(HOPPER_1_CREDIT_GPIO_Port, HOPPER_1_CREDIT_Pin);
		if (state != hopper_1_msp.credit_pin_state)
		{
			if (state == GPIO_PIN_RESET) {
			...
			hopper_1_msp.credit_pin_state = state;
		}

		state = HAL_GPIO_ReadPin(HOPPER_1_COIN_EMPTY_GPIO_Port, HOPPER_1_COIN_EMPTY_Pin);
		if (state != hopper_1_msp.coin_empty_pin_state)
		{
			if (state == GPIO_PIN_RESET) {
			...
			hopper_1_msp.coin_empty_pin_state = state;
		}
}
