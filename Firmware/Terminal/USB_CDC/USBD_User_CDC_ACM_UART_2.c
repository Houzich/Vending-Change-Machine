...
static void Transmit_Enable(void)
{
	...
	HAL_GPIO_WritePin(RS485_EN_2_GPIO_Port, RS485_EN_2_Pin, GPIO_PIN_RESET);
}
/*###############################################################*/
/*###############################################################* Receive_Enable -->*/
/*###############################################################*/
...
static void Receive_Enable(void)
{
	...
	HAL_GPIO_WritePin(RS485_EN_2_GPIO_Port, RS485_EN_2_Pin, GPIO_PIN_SET);
}
...
static uint64_t             cdc0_acm_uart_to_usb_thread_stack_mem[1024U / 8U] __SECTION(.bss.os.thread.stack);
#endif
...
void USBD_CDC0_ACM_Initialize (void) {
  ...
  ptrUART->Control(ARM_USART_MODE_ASYNCHRONOUS |
   ...
   ptrUART->Control (ARM_USART_CONTROL_TX, 1);
   ptrUART->Control (ARM_USART_CONTROL_RX, 1);
